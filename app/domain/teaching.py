...
def _facts_block(data):
    """Each fact, its evidence locator, and how far it has got.

    Section 3 asks for the fact, its locator, and whether it is
    asserted, extracted, confirmed or disputed. The distinction is the
    point of the section: a reviewer must be able to see at a glance
    that nothing here has been confirmed by a person yet.
    """
    if not data["facts"]:
    ...
    for predicate, value, status, origin, evidence in data["facts"]:
        label = data["hints"].get(predicate) or predicate
        shown = (value or "").strip() or "(no value recorded)"
        locator = ", ".join(evidence or []) or "no evidence locator"

        lines.append(
            f"- {label}\n"
            f"    value: {shown}\n"
            f"    status: {status} ({origin.lower().replace('_', ' ')})\n"
            f"    evidence: {locator}"
        )
