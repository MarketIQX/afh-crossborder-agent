...
196 checks across fourteen suites. Every one names what it proves, and the
...
Individual suites:
    ...
    tests/model_provider_smoke.py                MODEL01-MODEL19
...
`gap_smoke.py`, `guard_wiring_smoke.py`, `request_surface_smoke.py` and
`fact_authority_smoke.py` need no database and no credentials.
